...
dnl @version $Id: acinclude.m4,v 1.3 2004-03-16 12:21:19 dog Exp $
dnl
...
dnl @synopsis _INETLIB_FIND_JSSE_JAR
dnl
AC_DEFUN([_INETLIB_FIND_JSSE_JAR],[
AC_MSG_CHECKING([for jsse.jar])
for _F in $1 ;do
	if test -r ${_F}/jsse.jar ; then
		JSSE_JAR=${_F}/jsse.jar
		break
	elif test -r ${_F}/javax-net.jar ; then
		JSSE_JAR=${_F}/javax-net.jar
		break
	fi
done
if test x${JSSE_JAR} = x ; then
...
dnl @synopsis INETLIB_WITH_JAVAX_SECURITY_AUTH_CALLBACK_JAR
dnl
AC_DEFUN([INETLIB_WITH_JAVAX_SECURITY_AUTH_CALLBACK_JAR],[
AC_ARG_WITH([javax-security-auth-callback_jar],
	AC_HELP_STRING([--with-javax-security-auth-callback-jar=DIR],
		[path to GNU-Crypto javax.security.auth.callback library (javax-security-auth-callback.jar). If unspecified, /usr/share/java and /usr/local/share/java are searched, in that order.]),
	[if test "x${withval}" != x ; then
		AC_MSG_CHECKING([for ${withval}/javax-security-auth-callback.jar])
		if test -r ${withval}/javax-security-auth-callback.jar ; then
			AC_MSG_RESULT(yes)
			JAVAX_SECURITY_AUTH_CALLBACK_JAR=${withval}/javax-security-auth-callback.jar
			found_javax_security_auth_callback_jar=true
		else
			AC_MSG_ERROR([can't find javax-security-auth-callback.jar in specified path])
		fi
	fi],
	[found_javax_security_auth_callback_jar=false])
if test "x${JAVAX_SECURITY_AUTH_CALLBACK_JAR}" = x ; then
	dnl AC_MSG_NOTICE([no value supplied, searching default locations])
	_INETLIB_FIND_JAVAX_SECURITY_AUTH_CALLBACK_JAR([/usr/share/java /usr/local/share/java])
	if test "x${JAVAX_SECURITY_AUTH_CALLBACK_JAR}" = x; then
		AC_MSG_ERROR([can't find javax-security-auth-callback.jar; use --with-javax-security-auth-callback-jar])
	fi
fi
AC_SUBST(JAVAX_SECURITY_AUTH_CALLBACK_JAR)
])
dnl @synopsis _INETLIB_FIND_JAVAX_SECURITY_AUTH_CALLBACK_JAR
dnl
AC_DEFUN([_INETLIB_FIND_JAVAX_SECURITY_AUTH_CALLBACK_JAR],[
AC_MSG_CHECKING([for javax-security-auth-callback.jar])
for _F in $1 ;do
	if test -r ${_F}/javax-security-auth-callback.jar ; then
		JAVAX_SECURITY_AUTH_CALLBACK_JAR=${_F}/javax-security-auth-callback.jar
		break
	fi
done
if test x${JAVAX_SECURITY_AUTH_CALLBACK_JAR} = x ; then
	...
	AC_MSG_RESULT([${JAVAX_SECURITY_AUTH_CALLBACK_JAR}])
fi])
dnl @synopsis INETLIB_WITH_JAVAX_SECURITY_SASL_JAR
dnl
AC_DEFUN([INETLIB_WITH_JAVAX_SECURITY_SASL_JAR],[
AC_ARG_WITH([javax-security-sasl_jar],
	AC_HELP_STRING([--with-javax-security-sasl-jar=DIR],
		[path to GNU-Crypto javax.security.sasl library (javax-security-sasl.jar). If unspecified, /usr/share/java and /usr/local/share/java are searched, in that order.]),
	[if test "x${withval}" != x ; then
		AC_MSG_CHECKING([for ${withval}/javax-security-sasl.jar])
		if test -r ${withval}/javax-security-sasl.jar ; then
			AC_MSG_RESULT(yes)
			JAVAX_SECURITY_SASL_JAR=${withval}/javax-security-sasl.jar
			found_javax_security_sasl_jar=true
		else
			AC_MSG_ERROR([can't find javax-security-sasl.jar in specified path])
		fi
	fi],
	[found_javax_security_sasl_jar=false])
if test "x${JAVAX_SECURITY_SASL_JAR}" = x ; then
	dnl AC_MSG_NOTICE([no value supplied, searching default locations])
	_INETLIB_FIND_JAVAX_SECURITY_SASL_JAR([/usr/share/java /usr/local/share/java])
	if test "x${JAVAX_SECURITY_SASL_JAR}" = x; then
		AC_MSG_ERROR([can't find javax-security-sasl.jar; use --with-javax-security-sasl-jar])
	fi
fi
AC_SUBST(JAVAX_SECURITY_SASL_JAR)
])
dnl @synopsis _INETLIB_FIND_JAVAX_SECURITY_SASL_JAR
dnl
AC_DEFUN([_INETLIB_FIND_JAVAX_SECURITY_SASL_JAR],[
AC_MSG_CHECKING([for javax-security-sasl.jar])
for _F in $1 ;do
	if test -r ${_F}/javax-security-sasl.jar ; then
		JAVAX_SECURITY_SASL_JAR=${_F}/javax-security-sasl.jar
		break
	fi
done
if test x${JAVAX_SECURITY_SASL_JAR} = x ; then
	...
	AC_MSG_RESULT([${JAVAX_SECURITY_SASL_JAR}])
fi])
