...
extends MimeMessage 
{
   ...
   */
  protected MboxMessage(MboxFolder folder, InputStream in, int msgnum) 
   ...
   * This is called by the MboxStore.
   */
  protected MboxMessage(MboxFolder folder, RandomAccessFile file, int msgnum) 
  throws MessagingException 
  {
    super(folder, msgnum);
  ...
  throws IOException 
  {
    ...
    for (line = file.readLine(); line!=null; line = file.readLine()) 
    {
      ...
      if (fromIndex==0)
      { // line begins with From_, end of message
	content = out.toByteArray();
	break;
      }
      else
